...
@php
    ...
    $classes = [
        $base,
        $variants['orientation'][$orientation] ?? $variants['orientation']['horizontal'],
    ];
@endphp

<div role="group" data-slot="button-group" data-orientation="{{ $orientation }}" {{ $attributes->twMerge($classes) }}>
    {{ $slot }}
</div>
